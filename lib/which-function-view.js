...
export default class WhichFunctionView {
    ...
    checkCurrentFunctionPython() {
        ...
        for( i = cursorPosition.row; i >= 0; --i ) {
            ...
            console.log( text )
            var isFunctionLinePy = text.indexOf('def')
            var isFunctionLinePerl = text.indexOf('sub')
            if( isFunctionLinePy > -1 || isFunctionLinePerl > -1 ) {                
                text = text.split(' ')
                console.log('here', text)
                var n = text[1].indexOf( '(' )
                if( n > -1 ) {
                    text[1] = text[1].slice(0, n)
                }
                this.currentFunction = text[ 1 ]                
                break
            }
